...
class RealFakeDataset(Dataset):
    def __init__(self,  real_path, 
        ...
        stat_from = "imagenet" if arch.lower().startswith("imagenet") else "orign_CLIP_model"
...
if __name__ == '__main__':
    ...
    for dataset_path in (dataset_paths):
        ...
        dataset_name = dataset_path.get('key', 'custom_test')

        with open(os.path.join(opt.result_folder, 'ap.txt'), 'a') as f:
            f.write(dataset_name + ': ' + str(round(ap * 100, 2)) + '\n')

        with open(os.path.join(opt.result_folder, 'acc0.txt'), 'a') as f:
            f.write(dataset_name + ': ' + str(round(r_acc0 * 100, 2)) + ' ' + str(round(f_acc0 * 100, 2)) + ' ' + str(
                round(acc0 * 100, 2)) + '\n')

        with open(os.path.join(opt.result_folder, 'acc1_best.txt'), 'a') as f:
            f.write(dataset_name + ': ' + str(round(r_acc1 * 100, 2)) + ' ' + str(round(f_acc1 * 100, 2)) + ' ' + str(
                round(acc1 * 100, 2)) + ' thres=' + str(round(best_thres, 4)) + '\n')
